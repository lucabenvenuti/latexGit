...
numerical \ac{mupsh} from the \textit{training set} to create 36 $ANNs$ that
...
training.
Thus, we could select for \ac{mupsh} the $ANN$ with the maximum $R^2$, 
...
We repeated the same $ANN$ creation steps for \ac{mush}, \ac{rhob}
and \ac{AoR}, obtaining one trained $ANN$ for each bulk value. \\
Since \ac{mupsh}, \ac{mush} and \ac{rhob} belonged to the shear-cell
simulations, their $ANNs$ were handled together: we had one cluster with three 
$ANNs$ for the shear cell and one with only one $ANN$ for the \ac{AoR}.
...
cell and one for the \ac{AoR} were obtained.
% %************************************************
