...
There are ways to determine contact parameters directly by measuring material properties or 
by performing particle based experiments, see e.g. Combarros et al. \cite{RefWorks:177}, 
Paulick et al. \cite{RefWorks:181}, and Lommen et al. \cite{RefWorks:186}. However, these methodologies are laborious, 
since they have to be performed for every new granular material prior to a $DEM$
simulations. Especially for the rolling friction parameter, it is arduous to link the rolling 
friction parameter to the non-sphericity of the particle. Clearly, there is a
...
The material used for this work was sinter fine. Its particles had a mean radius of 0.73 mm and were cohesionless, 
see Section \ref{subsec:macroscopicexperimentsparameteridentification}. 
...
Due to the limited number of experiments realized, a restricted number of flow regimes were tested. 
Further, only one size distribution was considered. 
...
We chose an elastic model to picture the particle behaviour. If we try to identify parameters 
for an elastic contact model in a system of non-elastic particles, this $ANN$ approach will fail. 
So choosing appropriate contact models is an essential pre-requisite for contact 
parameter identification by means of $ANN$. 
Further, $ANNs$ can give indications about the correctness of the chosen model, 
...
calculates the force that particle i exerts on particle j. 
We decided to utilize a single contact law for all the simulations performed. 
...
percentage (40, 60, 80, 100 \%) during the incipient flow condition $\tau_{\%}$
...
in a specific flow regime for a range of bulk materials that can be represented
with a defined particle bulk behaviour.\\
...
\caption[Parameter space plot of valid simulations parameters for one
    ...
    parameters for one bulk behaviours measured by a shear cell
    tester ($SSC$).
	...
	The marked combinations for $\sigma_n=1068$ Pa are presented.
...
B.~Harthong, J.~F. Jérier, P.~Dorémus, D.~Imbault, F.~V. Donzé, Modeling of
