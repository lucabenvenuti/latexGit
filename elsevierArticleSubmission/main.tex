...
The amounts of open and closed pores of the particles are not negligible (see
Yang \cite{RefWorks:191}), and thus the use of an Archimedean procedure to
...
The high shape dispersity made not only this analysis impossible, but forbade us
to achieve a trustworthy average density for the particles in a plausible time.
Additionally, assigning a large number of different particle densities 
to the numerical entities is futile, if a thorough investigation of the  
correspondence between experimental and numerical bulk densities is not
performed.
However, this comparison was not included in the scope of our work.
...
Rather, further investigations on these two aspects could again demonstrate its
effectiveness.\\
Notably, we chose an elastic model to picture the particle behaviour. If we try
to identify parameters for an elastic contact model in a system of non-elastic particles, 
this $ANN$ approach will fail. 
So choosing appropriate contact models is an essential pre-requisite for contact 
parameter identification by means of $ANN$.
If we chose a wrong contact law, 
$ANNs$ could give indications about its correctness, 
see Section \ref{subsec:experimentsparameteridentification}, 
by identifying no valid parameters.
However, in the worst case scenario, we could end up adopting unreliable
parameters in the chosen law.
